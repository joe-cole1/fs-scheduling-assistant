...
The authorized release path is **Actions → Publish release** from the default branch with one version input (for example `0.5.3`). The workflow validates/builds/tests before release creation, generates the notes, creates or resumes only a matching mutable draft, verifies all release assets, then publishes the draft as the final action. Do not manually create/publish the normal release, move an existing tag, overwrite differing assets, or attempt to repair an already-published immutable release. A failed matching draft may be resumed by rerunning the same version; a published failure requires a new version.
