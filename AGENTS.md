...
The authorized release path is **Actions → Publish release** from the default branch. `packaging/version.txt` is the reviewed stable release identity. Normally leave the workflow Version field blank so the workflow uses that source version; an entered Version is only a confirmation and must match exactly. All publish runs are serialized. The workflow validates/builds/tests before release creation, generates notes, creates or resumes only a matching mutable draft, safely retries/reconciles transient asset-upload failures, verifies all release assets, then publishes the draft as the final action. Do not manually create/publish the normal release, move an existing tag, overwrite differing completed assets, or attempt to repair an already-published immutable release. A failed matching draft may be resumed by rerunning the same source version.