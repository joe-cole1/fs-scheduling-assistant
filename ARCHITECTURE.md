...
The single update ZIP contains System, START HERE.docx and update instructions. It excludes all local guidance and weekly folders by construction. Replacing System next week updates reusable instructions without rewriting local policy. Publishing a GitHub Release triggers the maintainer workflow to build from the exact tagged source, run structural/update checks, attach ZIPs plus manifest/checksums and append a generated download section to the release notes. A manual run can fill an existing published release. Generated downloads live in Releases, outside the repository tree; README links to the latest published assets. No branch writeback, automatic installation or shared-drive access exists. This publishing automation is not an operational dependency. See packaging/README.md for release notes, versioning, retries and the publish-before-build timing limitation.
