...
Release publication is a draft-first transaction owned by the **Publish release** workflow. From the default branch, the Version input may be left blank to increment the latest published stable release by `0.0.1`, or a maintainer may enter an explicit stable version for a deliberate choice. All publication runs are serialized. The workflow validates/builds/tests the exact default-branch source commit before release creation, generates structured notes and GitHub change/contributor notes, creates or resumes only a matching draft, uploads and byte-verifies all release assets, and publishes the draft only after its final preflight. Published immutable releases are never repaired or mutated; a failed matching draft can be safely resumed, and a blank retry resolves to the same next patch until publication succeeds. Generated downloads live in Releases, outside the repository tree. No branch writeback, automatic installation or shared-drive access exists.
