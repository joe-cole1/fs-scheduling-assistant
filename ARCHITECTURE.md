...
Release publication is a draft-first transaction owned by the **Publish release** workflow. From the default branch, a maintainer enters one version number. The workflow validates/builds/tests the exact default-branch source commit before release creation, generates structured notes and GitHub change/contributor notes, creates or resumes only a matching draft, uploads and byte-verifies all release assets, and publishes the draft only after its final preflight. Published immutable releases are never repaired or mutated; a failed matching draft can be safely resumed. Generated downloads live in Releases, outside the repository tree. No branch writeback, automatic installation or shared-drive access exists.
