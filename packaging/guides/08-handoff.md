...
the Session Handoff Word form. Do not claim you saved to our shared drive.
```

Review and save **Current Handoff.docx** in **Working Record**. If needed, paste into **08 Session Handoff.docx** from Blank Forms. Transfer the actual source files and decision record too. A transcript is optional and does not replace them.
...
handoff and actual source files. Reconcile versions and flag missing files or
conflicting state. Preserve human decisions and confirmations with their scope.
Tell me the current phase and next action, then continue my requested task.
Do not assume access to another user's files or conversation.
Apply the recorded test condition and cutoff. If prohibited hindsight is
present, stop the blind review and require an isolated clean restart.
```

Each chat sees only its supplied context. Humans reconcile competing proposals before identifying a new active or signed schedule. Never share CAC credentials.
