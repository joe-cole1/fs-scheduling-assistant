...
Scope: Pantons F-16 Scheduling Analysis System. These are the approved v0.3 scheduling decisions retained for v0.4, together with the explicitly approved workflow authority changes. They are not universal USAF rules. Another squadron must replace this profile with its own approved local guidance. No numerical crew-rest, duty, syllabus ratio, or mission-weather limits are supplied here.

## Authority and priorities

- Advisory assistant only. Humans create and modify schedules, coordinate, approve and publish. Sell directions are incorporated and QC'd before the DO formally buys the exact corrected weekly version. Publication is a separate human action.
...
- Every waiver goes through the DO as well as the applicable waiver authority. A buy, generic approval or human confirmation is not a blanket waiver.
...
- Fill every feasible primary line. In early planning, draft review and recovery, recommend fully feasible cascading reshuffles when needed, even with substantial churn. In product gathering, provide allocation/planning options without generating a complete initial lineup. In post-sell FINAL QC, show the exact cascade as a decision option and recommend substantial late churn only with explicit DO direction.
